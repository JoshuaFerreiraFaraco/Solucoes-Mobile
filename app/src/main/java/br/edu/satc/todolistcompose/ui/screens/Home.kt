...
@Composable
fun HomeScreen(
    tasksList: List<TaskData>,
    onNewTaskCreated: (task: TaskData) -> Unit,
    onTaskUpdate: (task: TaskData) -> Unit
) {
    ...
    Scaffold(
         ...
         * */
        HomeContent(innerPadding, tasksList) {
            onTaskUpdate(it)
        }
...
fun HomeContent(
    ...
    tasksList: List<TaskData>,
    tasksCompleted: (task: TaskData) -> Unit
) {
    /**
